...
export default function RouteGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (user && pathname === '/login') { router.replace('/dashboard'); return }
  ...
  if (loading) {
    return (
      <div style={{
        ...
        <div style={{
          width: 48, height: 48, background: '#4A90E2', borderRadius: 14,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 18, fontWeight: 800, color: '#fff', marginBottom: 8,
        }}>MQ</div>
        <div style={{ fontSize: 18, fontWeight: 700, color: '#fff' }}>MedQuest</div>
        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
          {[0,1,2].map(i => (
            <div key={i} style={{
              width: 8, height: 8, borderRadius: '50%', background: '#4A90E2',
